...
def get_dataloader(args):
    ...
    if args.dataset == 'btcv':
        ...
        '''brats data'''
        brats_train_dataset = BraTS(args, args.data_path, transform = None, transform_msk= None, mode = 'train', prompt=args.prompt)
        brats_test_dataset = BraTS(args, args.data_path, transform = None, transform_msk= None, mode = 'test', prompt=args.prompt)
        brats_valid_dataset = BraTS(args, args.data_path, transform = None, transform_msk= None, mode = 'valid', prompt=args.prompt)

        nice_train_loader = DataLoader(brats_train_dataset, batch_size=1, shuffle=True, num_workers=8, pin_memory=True)
        nice_test_loader = DataLoader(brats_test_dataset, batch_size=1, shuffle=False, num_workers=1, pin_memory=False)
        nice_valid_loader = DataLoader(brats_valid_dataset, batch_size=1, shuffle=False, num_workers=1, pin_memory=False)
        '''end'''

    else:
        print("the dataset is not supported now!!!")
        
    return nice_train_loader, nice_valid_loader,nice_test_loader